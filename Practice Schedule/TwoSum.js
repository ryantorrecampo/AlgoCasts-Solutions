const twoSum = (nums, target) => {
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        if (another in map) {
            // checks if the complement is in the table;
            return [map[another], i]; // return the index of the compliment and the current index
        }
        map[nums[i]] = i; // Creates a hash table where key is the number and value is the index
    }
    return null;
};
